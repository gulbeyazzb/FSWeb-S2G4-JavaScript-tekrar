...
// console.log("karenin alanı: " + KareninAlani(10) + " cm2");
...
// console.log("çemberin çevresi: " + CemberinCevresi(4) + " cm dir.");
...
function CemberinAlani(yariCap, pi) {
  const alan = Math.pow(yariCap, 2) * pi;
  return alan;
}
// console.log("Alan: " + CemberinAlani(4, pi));
...
// console.log("sayilar dizisinde: " + sayilar.length + " tane sayı var.");
...
// console.log("dizideki en büyük değer: " + enbuyuk);
// console.log("dizideki en küçük değer: " + enkucuk);
// 3b çözümü:

ucetambolunenler = [];
sayilar.forEach((value, index, sayilar) => {
  value % 3 == 0 ? ucetambolunenler.push(sayilar[index]) : null;
});
// console.log(ucetambolunenler);

//3c çözümü:

ucebolunenlerintoplami = ucetambolunenler.reduce((sum, value) => {
  return sum + value;
}, 0);
...
besyuzdenkucuksayilar = sayilar.filter((value) => {
  return value < 500;
});
...
for (let i = 0; i < sayilar.length; i++) {
  if (count[sayilar[i]]) {
    //Count nesnesinin içinde sayilar dizisinin i. index değeri var mı? Varsa
    count[sayilar[i]] += 1; //Değerin valuesını 1 arttır
  } else {
    count[sayilar[i]] = 1; //değer yoksa key olarak ata ve değerine 1 rakamını ata
  }
}
for (let number in count) {
  // nesnenin içindeki her key değerinde gezinecek bir number değişkeni tanımladık
  if (count[number] >= 2) {
    //Nesnenin number olarak temsil edilen key'inin değeri 2'ye eşit veya 2'den büyükse
    tekraredensayilar.push(
      `${number} sayısı ${count[number]} kere tekrar edilmiştir` //tekraredensayilar dizine pushla
    );
  }
}
